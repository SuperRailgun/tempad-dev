...
export const GetStructureParametersSchema = z.object({
  nodeId: z
    .string()
    .describe(
      'Optional node id to outline; accepts a page id to outline that whole page. Defaults to the current single selection, and falls back to the document page list when nothing is selected. Useful for explicit hierarchy/geometry, stable authoring keys on TemPad-managed nodes, and when auto-layout hints are none/inferred.'
    )
    .optional(),
  options: z
    .object({
