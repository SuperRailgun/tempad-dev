...
- `apply_canvas`: One restricted HTML + deterministic Tailwind utility desired result using primitives, catalog
  ...
  verifies the result.
- `get_structure` (also exposed as `get_metadata`): Hierarchy/geometry outline for the selection, including stable authoring keys on TemPad-managed nodes; for a whole page when given a page id, or the open document's page list when nothing is selected.
